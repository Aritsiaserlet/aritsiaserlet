...
function renderIconLibrary() {
  ...
  const starActiveIcon = document.getElementById('manageWorkStarActiveIcon');
  if(starActiveIcon) starActiveIcon.innerHTML = generateIconOptions(settings.manageWorkStarActiveIconId || '', '-- None --');
  const starInactiveIcon = document.getElementById('manageWorkStarInactiveIcon');
  if(starInactiveIcon) starInactiveIcon.innerHTML = generateIconOptions(settings.manageWorkStarInactiveIconId || '', '-- None --');
...
function renderAdminList(){
  ...
  displayList.forEach(w=>{
    ...
    let starActiveHtml = '⭐';
    if(settings.manageWorkStarActiveIconId && settings.icons) {
      const sIc = settings.icons.find(x => x.id === settings.manageWorkStarActiveIconId);
      if(sIc) starActiveHtml = `<img src="${sIc.url}" style="width:20px;height:20px;object-fit:cover;image-rendering:pixelated;">`;
    }

    let starInactiveHtml = '☆';
    if(settings.manageWorkStarInactiveIconId && settings.icons) {
      const sIc = settings.icons.find(x => x.id === settings.manageWorkStarInactiveIconId);
      if(sIc) starInactiveHtml = `<img src="${sIc.url}" style="width:20px;height:20px;object-fit:cover;image-rendering:pixelated;">`;
    }
    ...
    el.innerHTML=`
      ...
      <button class="witem-star" onclick="toggleStar(${w.id})" title="Feature in Dynamic Reel" style="font-size:20px; background:none; border:none; cursor:pointer; padding:4px;">${w.starred ? starActiveHtml : starInactiveHtml}</button>
...
window.toggleStar = async function(id) {
  const w = works.find(x => x.id === id);
  if(w) {
    // Optimistic UI Update (Real-time feedback)
    w.starred = !w.starred;
    renderAdminList(); 
